...
class PageCache:
    ...
    def __init__(self, RHManager, Events):
    ...
    def update_cache(self):
        ...
        if self.get_valid(): # Output existing calculated results
                ...
                if self._rhmanager.rhdata.savedRaceMetas_has_heat(heat.id):
                    ...
                    for race in self._rhmanager.rhdata.get_savedRaceMetas_by_heat(heat.id):    
                        ...
                        for pilotrace in self._rhmanager.rhdata.get_savedPilotRaces_by_savedRaceMeta(race.id):
                            gevent.sleep()
                            laps = []
                            for lap in self._rhmanager.rhdata.get_savedRaceLaps_by_savedPilotRace(pilotrace.id):
                                laps.append({
                                    'id': lap.id,
                        ...
                        results = self._rhmanager.rhdata.get_results_savedRaceMeta(race)
                        rounds.append({
                            'id': race.round_id,
                    ...
                    results = self._rhmanager.rhdata.get_results_heat(heat)
                    heats[heat.id] = {
                        'heat_id': heat.id,
            ...
            gevent.sleep()
            heats_by_class = {}
            heats_by_class[RHUtils.CLASS_ID_NONE] = [heat.id for heat in self._rhmanager.rhdata.get_heats_by_class(RHUtils.CLASS_ID_NONE)]
            for race_class in self._rhmanager.rhdata.get_raceClasses():
                heats_by_class[race_class.id] = [heat.id for heat in self._rhmanager.rhdata.get_heats_by_class(race_class.id)]
            ...
            current_classes = {}
            for race_class in self._rhmanager.rhdata.get_raceClasses():
                results = self._rhmanager.rhdata.get_results_raceClass(race_class)
                ranking = self._rhmanager.rhdata.get_ranking_raceClass(race_class)
                current_class = {}
            ...
            gevent.sleep()
            results = self._rhmanager.rhdata.get_results_event()
