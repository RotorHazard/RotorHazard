...
echo on
"%ARDUINO_CMD%" --verify --board STMicroelectronics:stm32:GenF4:pnum=BLACKPILL_F411CE --pref compiler.cpp.extra_flags=-DSTM32F4 --pref xserial=generic,usb=none,xusb=FS,opt=osstd,rtlib=nano --pref "build.path=%RH_BUILD_WORKDIR%build_stm32_f4" --pref build.project_name=%RH_BUILD_PROJNAME% "%RH_BUILD_WORKDIR%rhnode.cpp" %1 %2 %3 %4 %5 %6 %7 %8 %9
@echo off
:ex
