...
var freqTable = {
	...
	buildSelect: function() {
		...
		var keyNames = Object.keys(this);
		for (var i in keyNames) {
			if (typeof(this[keyNames[i]]) !== 'function') {
				if (this[keyNames[i]] == 0) {
					output += '<option value="0">Disabled</option>';
				} else if (this[keyNames[i]] == 'n/a') {
					output += '<option value="n/a">N/A</option>';
				} else {
					output += '<option value="' + this[keyNames[i]] + '">' + keyNames[i] + ' ' + this[keyNames[i]] + '</option>';
				}
			}
		}
		return output;
