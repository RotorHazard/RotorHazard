...
try:
	cursor.execute("""CREATE TABLE IF NOT EXISTS nodes (node INT, i2cAddr INT, vtxFreq INT, vtxFreqChan VARCHAR(255), rssi INT, rssiTrigger INT)""")
	db.commit()
except:
	db.rollback()
...
try:
	cursor.execute("""CREATE TABLE IF NOT EXISTS pilots (pilot INT, callSign VARCHAR(255), firstName VARCHAR(255), lastName VARCHAR(255))""")
	db.commit()
except:
	db.rollback()
...
try:
	cursor.execute("""CREATE TABLE IF NOT EXISTS groups (groups INT, pilot INT)""")
	db.commit()
except:
	db.rollback()
...
try:
	cursor.execute("""CREATE TABLE IF NOT EXISTS savedRaces (rount INT, groups INT, pilot INT, lap INT, min INT, sec INT, milliSec INT)""")
	db.commit()
except:
	db.rollback()
