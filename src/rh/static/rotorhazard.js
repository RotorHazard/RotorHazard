...
var freq = {
	frequencies: {},
	loadTable: function(vtx_table) {
		for (var band of vtx_table.vtx_table.bands_list) {
			for (var i = 0; i<band.frequencies.length; i++) {
				if (band.frequencies[i]) {
					freq.frequencies[band.letter+(i+1)] = band.frequencies[i];
				}
			}
		}
	},
...
function frequency_data_handler(msg) {
	freq.loadTable(msg.vtx_table);

	// populate any frequency selects
