...
Emitted once for each staging tone in the countdown. Each event is sent before its tone by `GENERAL > SYNC_LEADTIME_SECS`; plugins can use `scheduled_at_monotonic` to produce server-side staging sounds or trigger hardware effects in sync with each staging beep.
